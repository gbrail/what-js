...
promise_test(t => {
  ...
  var pipeToRejected = false;
  var pipeToPromise = promise_rejects_js(t, TypeError, rs.pipeTo(ts.writable), 'pipeTo should reject').then(() => {
    pipeToRejected = true;
  });
  return delay(0).then(() => {
...
promise_test(t => {
  ...
  var pipeToPromise = rs.pipeTo(ts.writable);
  return delay(0).then(() => {
...
promise_test(t => {
  ...
  return Promise.all([promise_rejects_js(t, TypeError, ts.writable.abort(), 'abort() should reject with a TypeError'), promise_rejects_exactly(t, error1, ts.readable.cancel(), 'cancel() should reject with error1'), promise_rejects_exactly(t, error1, ts.readable.getReader().closed, 'closed should reject with error1')]);
...
promise_test(t => {
  ...
  return Promise.all([promise_rejects_js(t, TypeError, ts.writable.abort(), 'abort() should reject with a TypeError'), ts.readable.cancel(), ts.readable.getReader().closed]);
...
promise_test(() => {
  ...
  return Promise.all([writer.close(), writer.closed, ts.readable.getReader().closed]);
