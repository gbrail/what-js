...
promise_test(() => {
  ...
  return Promise.all([writer.write('a'), writer.close()]).then(() => readableStreamToArray(ts.readable)).then(array => assert_array_equals(array, ['b', 'a'], 'array should contain two chunks'));
...
promise_test(() => {
  ...
  var writer = ts.writable.getWriter();
  return writer.write('a').then(() => readableStreamToArray(ts.readable)).then(array => assert_array_equals(array, [], 'array should contain no chunks'));
...
promise_test(t => {
  ...
  var writer = ts.writable.getWriter();
  return writer.write('a').then(() => promise_rejects_exactly(t, error1, ts.readable.getReader().read(), 'read() should reject'));
...
promise_test(() => {
  ...
  var writer = ts.writable.getWriter();
  return Promise.all([writer.write('a'), writer.close()]).then(() => readableStreamToArray(ts.readable)).then(array => assert_array_equals(array, ['a'], 'array should contain one chunk'));
...
promise_test(() => {
  ...
  var writer = ts.writable.getWriter();
  var writeResolved = false;
  var writePromise = writer.write('b').then(() => {
    writeResolved = true;
  });
