...
promise_test(t => {
  ...
  return Promise.all([promise_rejects_exactly(t, thrownError, writer.write('a'), 'writable\'s write should reject with the thrown error'), promise_rejects_exactly(t, thrownError, reader.read(), 'readable\'s read should reject with the thrown error'), promise_rejects_exactly(t, thrownError, reader.closed, 'readable\'s closed should be rejected with the thrown error'), promise_rejects_exactly(t, thrownError, writer.closed, 'writable\'s closed should be rejected with the thrown error')]);
...
promise_test(t => {
  ...
  var writer = ts.writable.getWriter();
  var reader = ts.readable.getReader();
  return Promise.all([promise_rejects_exactly(t, thrownError, writer.write('a'), 'writer should reject with thrownError'), promise_rejects_exactly(t, thrownError, writer.close(), 'close() should reject with thrownError'), promise_rejects_exactly(t, thrownError, reader.read(), 'reader should reject with thrownError')]);
}, 'TransformStream transformer.start() rejected promise should error the stream');
promise_test(t => {
  var controllerError = new Error('start failure');
  ...
  var writer = ts.writable.getWriter();
  var reader = ts.readable.getReader();
  return Promise.all([promise_rejects_exactly(t, controllerError, writer.write('a'), 'writer should reject with controllerError'), promise_rejects_exactly(t, controllerError, writer.close(), 'close should reject with same error'), promise_rejects_exactly(t, controllerError, reader.read(), 'reader should reject with same error')]);
...
test(() => {
  var controllerError = new URIError('controller.error');
  var controller;
  var strategy = {
    size() {
...
promise_test(t => {
  ...
  return Promise.all([ts.readable.cancel(thrownError), promise_rejects_exactly(t, thrownError, closedPromise, 'closed should throw a TypeError')]);
...
promise_test(t => {
  ...
  return Promise.all([promise_rejects_exactly(t, thrownError, reader.closed, 'reader.closed should reject'), promise_rejects_exactly(t, thrownError, writePromise, 'writePromise should reject'), promise_rejects_exactly(t, thrownError, closePromise, 'closePromise should reject')]);
...
promise_test(t => {
  ...
  var writePromise = ts.writable.getWriter().write('a');
  var closedPromise = ts.readable.getReader().closed;
  return Promise.all([promise_rejects_exactly(t, thrownError, writePromise, 'write() should reject'), promise_rejects_exactly(t, thrownError, closedPromise, 'reader.closed should reject')]);
...
promise_test(t => {
  ...
  // TransformStream yet.
  var abortPromise = writer.abort(thrownError);
  var cancelPromise = ts.readable.cancel(new Error('cancel reason'));
  return Promise.all([abortPromise, cancelPromise, promise_rejects_exactly(t, thrownError, writer.closed, 'writer.closed should reject')]);
...
promise_test(t => {
  ...
  var writer = ts.writable.getWriter();
  return delay(0).then(() => {
    var writePromise = writer.write();
    var abortPromise = writer.abort(thrownError);
    var cancelPromise = ts.readable.cancel(new Error('cancel reason'));
    resolveTransform();
    return Promise.all([writePromise, abortPromise, cancelPromise, promise_rejects_exactly(t, thrownError, writer.closed, 'writer.closed should reject with thrownError')]);
  });
...
var ignoredError = new Error('ignoredError');
...
promise_test(t => {
  ...
  var cancelPromise = ts.readable.cancel(ignoredError);
  controller.error(thrownError);
  return Promise.all([cancelPromise, promise_rejects_exactly(t, thrownError, ts.writable.getWriter().closed, 'closed should reject with thrownError')]);
...
promise_test(t => {
  ...
  return delay(0).then(() => {
    ...
    var writePromise = writer.write('b');
...
promise_test(t => {
  ...
  return delay(0).then(() => {
    ...
    var writePromise = writer.write(0);
    ...
    var readPromise = ts.readable.getReader().read();
    return Promise.all([promise_rejects_exactly(t, thrownError, readPromise, 'read() should reject'), promise_rejects_exactly(t, thrownError, writePromise, 'write() should reject'), abortPromise]);
  });
