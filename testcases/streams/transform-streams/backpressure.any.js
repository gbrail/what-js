...
var error1 = new Error('error1 message');
error1.name = 'error1';
promise_test(() => {
  var ts = recordingTransformStream();
...
promise_test(() => {
  ...
  return Promise.all(writePromises).then(() => {
    assert_array_equals(ts.events, ['transform', 0, 'transform', 1, 'transform', 2, 'transform', 3], 'all 4 events should be transformed');
  });
...
promise_test(() => {
  ...
  var events = [];
  var writerPromises = [writer.write('a').then(() => events.push('a')), writer.write('b').then(() => events.push('b')), writer.close().then(() => events.push('closed'))];
  return delay(0).then(() => {
...
promise_test(t => {
  ...
  var ts = new TransformStream();
  var pipePromise = rs.pipeTo(ts.writable);
  ts.readable.cancel(error1);
