...
function createErroredWritableStream(t) {
  return Promise.resolve().then(() => {
    ...
    return promise_rejects_exactly(t, error2, writer.closed, 'the writable stream must be errored with error2').then(() => {
      writer.releaseLock();
...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, error2, rs.pipeTo(ws), 'pipeTo must reject with the writable stream\'s error').then(() => {
    ...
    assert_array_equals(ws.events, []);
    return Promise.all([promise_rejects_exactly(t, error1, rs.getReader().closed, 'the readable stream must be errored with error1'), promise_rejects_exactly(t, error2, ws.getWriter().closed, 'the writable stream must be errored with error2')]);
  });
...
promise_test(t => {
  ...
  return createErroredWritableStream(t).then(ws => promise_rejects_exactly(t, error1, rs.pipeTo(ws), 'pipeTo must reject with the readable stream\'s error')).then(() => {
    assert_array_equals(rs.events, []);
...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, error1, rs.pipeTo(ws, {
    preventAbort: true
  }), 'pipeTo must reject with the readable stream\'s error').then(() => {
    assert_array_equals(rs.events, []);
    assert_array_equals(ws.events, []);
    return Promise.all([promise_rejects_exactly(t, error1, rs.getReader().closed, 'the readable stream must be errored with error1'), promise_rejects_exactly(t, error2, ws.getWriter().closed, 'the writable stream must be errored with error2')]);
  });
...
promise_test(t => {
  ...
  return createErroredWritableStream(t).then(ws => promise_rejects_exactly(t, error1, rs.pipeTo(ws, {
    preventAbort: true
  }), 'pipeTo must reject with the readable stream\'s error')).then(() => {
    assert_array_equals(rs.events, []);
    return promise_rejects_exactly(t, error1, rs.getReader().closed, 'the readable stream must be errored with error1');
...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, error1, rs.pipeTo(ws), 'pipeTo must reject with the readable stream\'s error').then(() => {
    ...
    return Promise.all([promise_rejects_exactly(t, error1, rs.getReader().closed, 'the readable stream must be errored with error1'), promise_rejects_exactly(t, error1, ws.getWriter().closed, 'closed must reject with error1'), promise_rejects_exactly(t, error1, closePromise, 'close() must reject with error1')]);
  });
...
promise_test(t => {
  ...
  return flushAsyncEvents().then(() => {
    return promise_rejects_exactly(t, error1, rs.pipeTo(ws), 'pipeTo must reject with the readable stream\'s error').then(() => {
      ...
      return Promise.all([promise_rejects_exactly(t, error1, rs.getReader().closed, 'the readable stream must be errored with error1'), ws.getWriter().closed, closePromise]);
    });
  });
...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, error1, rs.pipeTo(ws), 'pipeTo must reject with the writable stream\'s error').then(() => {
    assert_array_equals(rs.events, []);
    assert_array_equals(ws.events, []);
    return Promise.all([rs.getReader().closed, promise_rejects_exactly(t, error1, ws.getWriter().closed, 'the writable stream must be errored with error1')]);
  });
...
promise_test(t => {
  ...
  return createErroredWritableStream(t).then(ws => promise_rejects_exactly(t, error2, rs.pipeTo(ws), 'pipeTo must reject with the writable stream\'s error')).then(() => {
    assert_array_equals(rs.events, []);
