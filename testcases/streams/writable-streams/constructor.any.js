...
promise_test(t => {
  ...
  var writer = ws.getWriter();
  return Promise.all([writer.write('a'), promise_rejects_exactly(t, error1, writer.closed, 'controller.error() in write() should error the stream')]);
...
test(() => {
  assert_throws_js(RangeError, () => new WritableStream({
    type: 'bytes'
...
test(() => {
  ...
  assert_throws_js(TypeError, () => new WritableStreamDefaultController({}), 'constructor should throw a TypeError exception');
...
test(() => {
  ...
  assert_throws_js(TypeError, () => new WritableStreamDefaultController(stream), 'constructor should throw a TypeError exception');
...
test(() => {
  ...
  writer.releaseLock();
  assert_throws_js(TypeError, () => new WritableStreamDefaultWriter({}), 'constructor should throw a TypeError exception');
}, 'WritableStreamDefaultWriter should throw unless passed a WritableStream');
test(() => {
  var stream = new WritableStream();
  var writer = stream.getWriter();
  var WritableStreamDefaultWriter = writer.constructor;
  assert_throws_js(TypeError, () => new WritableStreamDefaultWriter(stream), 'constructor should throw a TypeError exception');
