...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, error1, writer.close(), 'close() promise must reject with the thrown error').then(() => promise_rejects_exactly(t, error1, writer.ready, 'ready promise must reject with the thrown error')).then(() => promise_rejects_exactly(t, error1, writer.closed, 'closed promise must reject with the thrown error')).then(() => {
...
promise_test(t => {
  ...
  var writer = ws.getWriter();
  return promise_rejects_exactly(t, error1, writer.close(), 'close() promise must reject with the same error').then(() => promise_rejects_exactly(t, error1, writer.ready, 'ready promise must reject with the same error')).then(() => assert_array_equals(ws.events, ['close']));
...
promise_test(t => {
  ...
  var writer = ws.getWriter();
  return promise_rejects_exactly(t, error1, writer.write('a'), 'write should reject with the thrown error').then(() => promise_rejects_exactly(t, error1, writer.closed, 'closed should reject with the thrown error'));
}, 'write: throwing method should cause write() and closed to reject');
promise_test(t => {
  var rejectSinkWritePromise;
  var ws = recordingWritableStream({
    write() {
  ...
  return flushAsyncEvents().then(() => {
    ...
    return Promise.all([promise_rejects_exactly(t, error1, writePromise, 'writer write must reject with the same error'), promise_rejects_exactly(t, error1, writer.ready, 'ready promise must reject with the same error')]);
  }).then(() => {
...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, error1, writer.write('b'), 'second write must reject with the same error').then(() => {
    assert_equals(writer.ready, readyPromise, 'the ready promise must not change, since the queue was full after the first write, so the pending one simply ' + 'transitioned');
...
promise_test(t => {
  ...
  var writer = ws.getWriter();
  return promise_rejects_exactly(t, error1, writer.abort(abortReason), 'abort should reject with the thrown error').then(() => promise_rejects_exactly(t, abortReason, writer.closed, 'closed should reject with abortReason'));
