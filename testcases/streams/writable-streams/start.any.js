...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, error1, ws.getWriter().write('a'), 'write() should reject with the error').then(() => {
...
promise_test(t => {
  ...
  controller.error(error1);
  resolveStart();
  return Promise.all([promise_rejects_exactly(t, error1, writePromise, 'write() should fail'), promise_rejects_exactly(t, error1, closePromise, 'close() should fail')]).then(() => {
