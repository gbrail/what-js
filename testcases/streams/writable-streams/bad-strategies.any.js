...
var error1 = new Error('a unique string');
error1.name = 'error1';
test(() => {
...
test(() => {
  var _loop = function (highWaterMark) {
...
promise_test(t => {
  ...
  var p1 = promise_rejects_exactly(t, error1, writer.write('a'), 'write should reject with the thrown error');
  var p2 = promise_rejects_exactly(t, error1, writer.closed, 'closed should reject with the thrown error');
  return Promise.all([p1, p2]);
}, 'Writable stream: throwing strategy.size method');
promise_test(() => {
  var sizes = [NaN, -Infinity, Infinity, -1];
