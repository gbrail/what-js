...
promise_test(t => {
  ...
  var cancel = delay(5).then(() => delay(5)).then(() => delay(5)).then(() => {
...
test(() => {
  ...
  assert_equals(recordedReason, passedReason, 'the error passed to the underlying source\'s cancel method should equal the one passed to the stream\'s cancel');
...
promise_test(() => {
  ...
  return rs.cancel().then(() => {
    ...
    }, 'read() should still work after the attempted cancel');
...
promise_test(t => {
  ...
  var rs = new ReadableStream({
    cancel() {
      var sourceCancelPromise = new Promise(resolve => resolveSourceCancelPromise = resolve);
...
promise_test(t => {
  ...
  var rs = new ReadableStream({
    cancel() {
      var sourceCancelPromise = new Promise((resolve, reject) => rejectSourceCancelPromise = reject);
...
promise_test(_async(function () {
  ...
  return _await(delay(0), function () {
    return _await(Promise.all([reader.cancel(), reader.closed]), function () {
