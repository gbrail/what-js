...
test(() => {
  var rsReader = new ReadableStreamDefaultReader(new ReadableStream());
...
test(() => {
  ...
  assert_throws_js(TypeError, () => new ReadableStreamDefaultReader(rs), 'constructing directly the second time should fail');
...
promise_test(() => {
  ...
  var promise = reader.read().then(result => {
    ...
    }, 'read() should fulfill with the enqueued chunk');
  });
...
promise_test(t => {
  ...
  controller.close();
  return Promise.all([promise_rejects_js(t, TypeError, reader1.closed), reader2.closed]);
...
promise_test(t => {
  ...
  var reader = rs.getReader();
  var promise1 = reader.closed;
  controller.close();
  reader.releaseLock();
  var promise2 = reader.closed;
  assert_not_equals(promise1, promise2, '.closed should be replaced');
  return Promise.all([promise1, promise_rejects_js(t, TypeError, promise2, '.closed after releasing lock')]);
...
promise_test(t => {
  ...
  var promise2 = reader.closed;
  assert_not_equals(promise1, promise2, '.closed should be replaced');
  return Promise.all([promise_rejects_exactly(t, theError, promise1, '.closed before releasing lock'), promise_rejects_js(t, TypeError, promise2, '.closed after releasing lock')]);
...
promise_test(() => {
  ...
  var reader1 = rs.getReader();
  var promise1 = reader1.read().then(r => {
    assert_object_equals(r, {
      value: 'a',
      done: false
    }, 'reading the first chunk from reader1 works');
  });
  reader1.releaseLock();
  var reader2 = rs.getReader();
  var promise2 = reader2.read().then(r => {
    assert_object_equals(r, {
      value: 'b',
      done: false
    }, 'reading the second chunk from reader2 works');
  });
  reader2.releaseLock();
...
promise_test(t => {
  ...
  var reader2 = rs.getReader();
  var readPromise = reader2.read().then(r => {
    assert_object_equals(r, {
      value: 'a',
      done: false
    }, 'a new reader should be able to read a chunk');
  });
  return Promise.all([promise_rejects_js(t, TypeError, cancelPromise), readPromise]);
...
promise_test(t => {
  ...
  var reader1 = rs.getReader();
  promiseAsserts.push(promise_rejects_exactly(t, theError, reader1.closed), promise_rejects_exactly(t, theError, reader1.read()));
  assert_throws_js(TypeError, () => rs.getReader(), 'trying to get another reader before erroring should throw');
  controller.error(theError);
  reader1.releaseLock();
  var reader2 = rs.getReader();
  promiseAsserts.push(promise_rejects_exactly(t, theError, reader2.closed), promise_rejects_exactly(t, theError, reader2.read()));
...
promise_test(t => {
  ...
  var promise = rs.getReader().closed.then(t.unreached_func('closed promise should not be fulfilled when stream is errored'), err => {
...
promise_test(t => {
  ...
  return rs.getReader().read().then(t.unreached_func('read promise should not be fulfilled when stream is errored'), err => {
    assert_equals(err, undefined, 'passed error should be undefined as it was');
  });
...
promise_test(t => {
  ...
  var promise = promise_rejects_exactly(t, theError, rs.getReader().closed);
  controller.error(theError);
  return promise;
...
promise_test(() => {
  ...
  var reader = rs.getReader();
  var promise = Promise.all([reader.read().then(result => {
    assert_object_equals(result, {
      value: undefined,
      done: true
    }, 'read() should fulfill with close (1)');
  }), reader.read().then(result => {
    assert_object_equals(result, {
      value: undefined,
      done: true
    }, 'read() should fulfill with close (2)');
  }), reader.closed]);
  controller.close();
  return promise;
...
promise_test(() => {
  ...
  controller.close();
  var reader = rs.getReader();
  return Promise.all([reader.read().then(result => {
    assert_object_equals(result, {
      value: undefined,
      done: true
    }, 'read() should fulfill with close (1)');
...
promise_test(t => {
  ...
  var reader = rs.getReader();
  return Promise.all([promise_rejects_exactly(t, myError, reader.read()), promise_rejects_exactly(t, myError, reader.read()), promise_rejects_exactly(t, myError, reader.closed)]);
...
promise_test(t => {
  ...
  var reader = rs.getReader();
  var promise = Promise.all([promise_rejects_exactly(t, myError, reader.read()), promise_rejects_exactly(t, myError, reader.read()), promise_rejects_exactly(t, myError, reader.closed)]);
  controller.error(myError);
  return promise;
...
promise_test(t => {
  ...
  var reader1 = rs.getReader();
  var promise1 = promise_rejects_js(t, TypeError, reader1.read(), 'read() from reader1 should reject when reader1 is released');
  reader1.releaseLock();
  controller.enqueue('a');
  var reader2 = rs.getReader();
  var promise2 = reader2.read().then(r => {
    ...
    }, 'read() from reader2 should resolve with enqueued chunk');
  });
  reader2.releaseLock();
  return Promise.all([promise1, promise2]);
