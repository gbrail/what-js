...
test(() => {
  ...
  new ReadableStream({
    type: undefined
  }); // ReadableStream constructed with undefined type
...
test(() => {
  ...
  assert_throws_exactly(error1, () => new ReadableStream({
    type: {
      get toString() {
        throw error1;
      }
...
test(() => {
  ...
  var source = {
    start(controller) {
      ...
      assert_array_equals(Object.getOwnPropertyNames(Object.getPrototypeOf(controller)).sort(), properties, 'prototype should have the right properties');
      controller.test = '';
      assert_array_equals(Object.getOwnPropertyNames(Object.getPrototypeOf(controller)).sort(), properties, 'prototype should still have the right properties');
      assert_not_equals(Object.getOwnPropertyNames(controller).indexOf('test'), -1, '"test" should be a property of the controller');
      startCalled = true;
    }
  };
  new ReadableStream(source);
...
promise_test(() => {
  ...
  return Promise.all([reader.read(), reader.read(), reader.read(), reader.closed]).then(r => {
    assert_object_equals(r[0], {
      value: objects[0],
      done: false
    }, 'value read should be the one enqueued');
    assert_object_equals(r[1], {
      value: objects[1],
      done: false
    }, 'value read should be the one enqueued');
    assert_object_equals(r[2], {
      value: objects[2],
      done: false
    }, 'value read should be the one enqueued');
  });
...
promise_test(() => {
  ...
  var closed = false;
  var read = false;
  return Promise.all([reader.closed.then(() => {
...
test(() => {
  var Subclass = /*#__PURE__*/function (_ReadableStream) {
    function Subclass() {
      _classCallCheck(this, Subclass);
      return _callSuper(this, Subclass, arguments);
    }
    _inherits(Subclass, _ReadableStream);
  ...
  assert_equals(Object.getPrototypeOf(Subclass.prototype), ReadableStream.prototype, 'Subclass.prototype\'s prototype should be ReadableStream.prototype');
  assert_equals(Object.getPrototypeOf(Subclass), ReadableStream, 'Subclass\'s prototype should be ReadableStream');
  var sub = new Subclass();
  assert_true(sub instanceof ReadableStream, 'Subclass object should be an instance of ReadableStream');
  assert_true(sub instanceof Subclass, 'Subclass object should be an instance of Subclass');
  var lockedGetter = Object.getOwnPropertyDescriptor(ReadableStream.prototype, 'locked').get;
