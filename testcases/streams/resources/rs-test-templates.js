...
self.templatedRSEmpty = (label, factory) => {
  ...
  test(() => {
    var rs = factory();
    assert_throws_js(TypeError, () => rs.getReader({
      mode: ''
...
self.templatedRSClosed = (label, factory) => {
  ...
  promise_test(() => {
    ...
    return Promise.all([cancelPromise1.then(v => assert_equals(v, undefined, 'first cancel() call should fulfill with undefined')), cancelPromise2.then(v => assert_equals(v, undefined, 'second cancel() call should fulfill with undefined'))]);
  ...
  test(() => {
    ...
    reader.releaseLock();
    var reader2 = rs.getReader(); // Getting a second reader should not throw.
    reader2.releaseLock();
...
self.templatedRSErrored = (label, factory, error) => {
  ...
  promise_test(t => {
    var rs = factory();
    var reader = rs.getReader();
    return Promise.all([promise_rejects_exactly(t, error, reader.closed), promise_rejects_exactly(t, error, reader.read())]);
  }, label + ': getReader() should return a reader that acts errored');
  promise_test(t => {
    var rs = factory();
    var reader = rs.getReader();
    return Promise.all([promise_rejects_exactly(t, error, reader.read()), promise_rejects_exactly(t, error, reader.read()), promise_rejects_exactly(t, error, reader.closed)]);
...
self.templatedRSErroredSyncOnly = (label, factory, error) => {
  ...
  promise_test(t => {
    ...
    var cancelPromise2 = rs.cancel();
    assert_not_equals(cancelPromise1, cancelPromise2, 'cancel() calls should return distinct promises');
    return Promise.all([promise_rejects_exactly(t, error, cancelPromise1), promise_rejects_exactly(t, error, cancelPromise2)]);
...
self.templatedRSEmptyReader = (label, factory) => {
  ...
  promise_test(t => {
    ...
    read().then(t.unreached_func('read() should not fulfill'), t.unreached_func('read() should not reject'));
    return delay(500);
  ...
  promise_test(_async(function (t) {
    ...
    return _awaitIgnored(Promise.all([promise_rejects_js(t, TypeError, read1, 'first read should reject'), promise_rejects_js(t, TypeError, read2, 'second read should reject'), promise_rejects_js(t, TypeError, closed, 'closed should reject')]));
  ...
  promise_test(t => {
    ...
    reader.releaseLock();
    return Promise.all([promise_rejects_js(t, TypeError, read()), promise_rejects_js(t, TypeError, read())]);
...
self.templatedRSClosedReader = (label, factory) => {
  ...
  promise_test(() => {
    var reader = factory().reader;
    return Promise.all([reader.read().then(v => {
      assert_object_equals(v, {
        value: undefined,
        done: true
      }, 'read() should fulfill correctly');
  ...
  promise_test(t => {
    ...
    return Promise.all([closedBefore.then(v => assert_equals(v, undefined, 'reader.closed acquired before release should fulfill')), promise_rejects_js(t, TypeError, closedAfter)]);
  ...
  promise_test(() => {
    ...
    return Promise.all([cancelPromise1.then(v => assert_equals(v, undefined, 'first cancel() should fulfill with undefined')), cancelPromise2.then(v => assert_equals(v, undefined, 'second cancel() should fulfill with undefined'))]);
  }, label + ': cancel() should return a distinct fulfilled promise each time');
};
...
self.templatedRSTwoChunksOpenReader = (label, factory, chunks) => {
  ...
  promise_test(() => {
    var reader = factory().reader;
    return Promise.all([reader.read().then(r => {
      assert_object_equals(r, {
        value: chunks[0],
        done: false
  ...
  promise_test(() => {
    ...
    reader.cancel();
    var promise3 = reader.read().then(r => {
      assert_object_equals(r, {
        value: undefined,
        done: true
...
self.templatedRSTwoChunksClosedReader = function (label, factory, chunks) {
  ...
  promise_test(() => {
    ...
    var promise = reader.closed.then(v => {
      assert_equals(v, undefined, 'reader closed should fulfill with undefined');
  ...
  promise_test(() => {
    ...
    var stream = streamAndReader.stream;
    var reader = streamAndReader.reader;
    var promise = reader.closed.then(() => {
  ...
  promise_test(t => {
    var reader = factory().reader;
    reader.releaseLock();
    return Promise.all([promise_rejects_js(t, TypeError, reader.read()), promise_rejects_js(t, TypeError, reader.read()), promise_rejects_js(t, TypeError, reader.read())]);
  }, label + ': releasing the lock should cause further read() calls to reject with a TypeError');
  promise_test(() => {
    var streamAndReader = factory();
    var stream = streamAndReader.stream;
...
self.templatedRSTeeCancel = (label, factory) => {
  ...
  promise_test(_async(function () {
    ...
    return _awaitIgnored(Promise.all([branch1.cancel(reason1), branch2.cancel(reason2), promise]));
  }), `${label}: canceling both branches should aggregate the cancel reasons into an array`);
  promise_test(_async(function () {
    var reason1 = new Error('This little one\'s not worth the effort.');
    var reason2 = new Error('Come, let me get you something.');
    var resolve;
    var promise = new Promise(r => resolve = r);
    var rs = factory({
      cancel(reason) {
    ...
    var [branch1, branch2] = rs.tee();
    return _awaitIgnored(Promise.all([branch2.cancel(reason2), branch1.cancel(reason1), promise]));
  ...
  promise_test(_async(function (t) {
    ...
    var [branch1, branch2] = rs.tee();
    return _awaitIgnored(Promise.all([promise_rejects_exactly(t, theError, branch1.cancel()), promise_rejects_exactly(t, theError, branch2.cancel())]));
  }), `${label}: failing to cancel the original stream should cause cancel() to reject on branches`);
  promise_test(_async(function (t) {
    var theError = {
      name: 'You just watch yourself!'
    };
    var controller;
    var stream = factory({
      start(c) {
