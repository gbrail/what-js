...
(() => {
  ...
  function testTransferredReadableStream(rs) {
    assert_equals(rs.constructor, ReadableStream, 'rs should be a ReadableStream in this realm');
    assert_true(rs instanceof ReadableStream, 'instanceof check should pass');

    // Perform a brand-check on |rs| in the process of calling getReader().
    var reader = ReadableStream.prototype.getReader.call(rs);
  ...
  function createTransferredReadableStream(underlyingSource) {
    var original = new ReadableStream(underlyingSource);
  ...
  function recordingTransferredReadableStream(underlyingSource, strategy) {
    var original = recordingReadableStream(underlyingSource, strategy);
    var promise = new Promise((resolve, reject) => {
      addEventListener('message', msg => {
        var rs = msg.data;
        if (rs instanceof ReadableStream) {
