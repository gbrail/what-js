...
test(() => {
  new ReadableStream({
    start(c) {
      var view = new Uint8Array(new ArrayBuffer(10), 0, 0);
      assert_throws_js(TypeError, () => c.enqueue(view));
    },
    type: 'bytes'
  });
...
async_test(t => {
  var stream = new ReadableStream({
    pull: t.step_func_done(c => {
      ...
      assert_throws_js(TypeError, () => c.byobRequest.respond(1), 'respond() must throw if the corresponding view has become detached');
    }),
    type: 'bytes'
  });
  var reader = stream.getReader({
...
async_test(t => {
  var stream = new ReadableStream({
    pull: t.step_func_done(c => {
      c.close();
      c.byobRequest.view.buffer.transfer();
      assert_throws_js(TypeError, () => c.byobRequest.respond(0), 'respond() must throw if the corresponding view has become detached');
    }),
    type: 'bytes'
  });
  var reader = stream.getReader({
...
async_test(t => {
  var stream = new ReadableStream({
    pull: t.step_func_done(c => {
      var view = c.byobRequest.view.subarray(1, 2);
...
async_test(t => {
  var stream = new ReadableStream({
    pull: t.step_func_done(c => {
      c.close();
      var view = c.byobRequest.view.subarray(1, 1);
      assert_throws_js(RangeError, () => c.byobRequest.respondWithNewView(view));
    }),
    type: 'bytes'
  });
...
async_test(t => {
  var stream = new ReadableStream({
    pull: t.step_func_done(c => {
      var view = new Uint8Array(new ArrayBuffer(10), 0, 3);
      assert_throws_js(RangeError, () => c.byobRequest.respondWithNewView(view));
    }),
    type: 'bytes'
  });
...
async_test(t => {
  ...
  var stream = new ReadableStream({
    pull: t.step_func_done(c => {
      var view = new Uint8Array(new ArrayBuffer(11), 0, 3);
      assert_throws_js(RangeError, () => c.byobRequest.respondWithNewView(view));
    }),
    type: 'bytes',
...
async_test(t => {
  var stream = new ReadableStream({
    pull: t.step_func_done(c => {
      var view = new Uint8Array(c.byobRequest.view.buffer, 0, 4);
      view[0] = 20;
      view[1] = 21;
      view[2] = 22;
      view[3] = 23;
  ...
  var view = new Uint8Array(buffer, 0, 3);
  view[0] = 10;
  view[1] = 11;
  view[2] = 12;
  reader.read(view);
...
async_test(t => {
  var stream = new ReadableStream({
    pull: t.step_func_done(c => {
      c.byobRequest.view.buffer.transfer();
      assert_throws_js(TypeError, () => c.enqueue(new Uint8Array([1])), 'enqueue() must throw if the BYOB request\'s buffer has become detached');
    }),
    type: 'bytes'
  });
  var reader = stream.getReader({
