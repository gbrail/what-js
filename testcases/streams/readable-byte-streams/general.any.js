...
error1.name = 'error1';
test(() => {
  assert_throws_js(TypeError, () => new ReadableStream().getReader({
    mode: 'byob'
  }));
...
test(() => {
  var ReadableStreamBYOBReader = new ReadableStream({
    type: 'bytes'
  }).getReader({
    mode: 'byob'
  }).constructor;
...
promise_test(() => {
  ...
  var startCalledBeforePull = false;
  var desiredSize;
  var controller;
...
test(() => {
  assert_throws_js(Error, () => new ReadableStream({
    start() {
      throw new Error();
    },
    type: 'bytes'
...
promise_test(t => {
  ...
  return reader.closed.then(() => {
    assert_throws_js(TypeError, () => stream.getReader({
      mode: 'byob'
    }), 'getReader() must throw');
  });
...
promise_test(_async(function (t) {
  ...
  // Should be a no-op
  reader1.releaseLock();
  return _await(reader2.read(new Uint8Array([0, 0, 0])), function (result) {
    assert_typed_array_equals(result.value, new Uint8Array([1, 2, 3]), 'read() should still work on reader2 even after reader1 is released');
...
promise_test(_async(function (t) {
  ...
  reader.releaseLock();
  return _awaitIgnored(promise_rejects_js(t, TypeError, read, 'pending read must reject'));
...
promise_test(() => {
  ...
  return Promise.resolve().then(() => {
    ...
    var byobRequest = byobRequests[1];
...
promise_test(() => {
  var pullCount = 0;
  var controller;
  var desiredSizeInStart;
  var desiredSizeInPull;
  var stream = new ReadableStream({
    start(c) {
...
promise_test(() => {
  ...
  var reader = stream.getReader();
  var promise = reader.read().then(result => {
...
promise_test(() => {
  ...
  var stream = new ReadableStream({
    ...
    pull() {
      ...
      var initialByobRequest = controller.byobRequest;
...
promise_test(() => {
  ...
  var stream = new ReadableStream({
    ...
    pull() {
      byobRequestDefined.push(controller.byobRequest !== null);
      var initialByobRequest = controller.byobRequest;
      var transferredView = transferArrayBufferView(controller.byobRequest.view);
...
promise_test(() => {
  var controller;
  var byobRequestWasDefined;
  var incorrectRespondException;
  var stream = new ReadableStream({
    start(c) {
      controller = c;
    },
...
promise_test(() => {
  ...
  var read1 = reader.read(new Uint16Array(2));
...
promise_test(t => {
  ...
  var readPromise = reader.read(new Uint8Array(1)).then(result => {
...
promise_test(() => {
  ...
  return Promise.resolve().then(() => {
    ...
    var promise = reader.read(new Uint16Array(1)).then(result => {
      assert_true(result.done, 'result.done');
      assert_equals(result.value, undefined, 'result.value');
    });
...
promise_test(() => {
  ...
  return Promise.resolve().then(() => {
    ...
    var promise = reader.read(new Uint16Array(2)).then(result => {
      assert_false(result.done, 'done');
      var view = result.value;
      ...
      var p = reader.read(new Uint16Array(1));
      assert_equals(pullCount, 1);
      return p;
    }).then(result => {
      assert_false(result.done, 'done');
...
promise_test(t => {
  ...
  return promise_rejects_js(t, TypeError, reader.read(new Uint16Array(1)), 'read(view) must fail').then(() => promise_rejects_js(t, TypeError, reader.closed, 'reader.closed should reject'));
...
promise_test(t => {
  ...
  var readPromise = reader.read(new Uint16Array(1));
  assert_throws_js(TypeError, () => controller.close(), 'controller.close() must throw');
  return promise_rejects_js(t, TypeError, readPromise, 'read(view) must fail').then(() => promise_rejects_js(t, TypeError, reader.closed, 'reader.closed must reject'));
...
promise_test(() => {
  ...
  var reader = stream.getReader();
  var p0 = reader.read().then(result => {
    assert_equals(pullCount, 1);
  ...
  var p1 = reader.read().then(result => {
    assert_equals(pullCount, 1);
    assert_false(result.done);
    var view = result.value;
    assert_equals(view.constructor, Uint8Array);
...
promise_test(t => {
  ...
  var p0 = reader.read(new Uint8Array(16)).then(result => {
  ...
  var p1 = reader.read(new Uint8Array(32)).then(result => {
...
promise_test(t => {
  ...
  var p1 = reader.read(new Uint8Array(16)).then(result => {
...
promise_test(t => {
  ...
  return promise_rejects_js(t, TypeError, reader.read(), 'read() must fail').then(() => assert_false(pullCalled, 'pull() must not have been called'));
...
promise_test(t => {
  ...
  return promise_rejects_js(t, TypeError, reader.read({
...
promise_test(t => {
  ...
  var reader = stream.getReader();
  var promise = promise_rejects_exactly(t, error1, reader.read(), 'read() must fail');
...
promise_test(t => {
  ...
  var promise = promise_rejects_exactly(t, error1, reader.read(new Uint8Array(1)), 'read() must fail');
  controller.error(error1);
  return promise;
...
promise_test(t => {
  var controller;
  var byobRequest;
  var testError = new TypeError('foo');
  var stream = new ReadableStream({
    start(c) {
      controller = c;
    },
  ...
  var reader = stream.getReader();
  var promise = promise_rejects_exactly(t, testError, reader.read(), 'read() must fail');
  return promise_rejects_exactly(t, testError, promise.then(() => reader.closed)).then(() => assert_equals(byobRequest, null, 'byobRequest must be null'));
...
promise_test(t => {
  ...
  var reader = stream.getReader();
  return promise_rejects_exactly(t, error1, reader.read(), 'read() must fail').then(() => promise_rejects_exactly(t, error1, reader.closed, 'closed must fail')).then(() => assert_equals(byobRequest, null, 'byobRequest must be null'));
...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, testError, reader.read(new Uint8Array(1)), 'read(view) must fail').then(() => promise_rejects_exactly(t, testError, reader.closed, 'reader.closed must reject')).then(() => assert_not_equals(byobRequest, null, 'byobRequest must not be null'));
...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, error1, reader.read(new Uint8Array(1)), 'read(view) must fail').then(() => promise_rejects_exactly(t, error1, reader.closed, 'closed must fail')).then(() => assert_not_equals(byobRequest, null, 'byobRequest must not be null'));
...
promise_test(() => {
  ...
  return reader.read(newView()).then(() => {
    assert_throws_js(TypeError, () => byobRequest.respondWithNewView(newView()), 'respondWithNewView() should throw a TypeError');
  });
...
promise_test(() => {
  ...
  var readPromise = reader.read(new Uint8Array(16));
...
promise_test(() => {
  ...
  var readPromise = reader.read(new Uint8Array(16));
  return pullCalledPromise.then(() => {
    var cancelPromise = reader.cancel('meh');
    assert_throws_js(TypeError, () => byobRequest.respond(0), 'respond() should throw');
    resolvePull();
    return Promise.all([readPromise, cancelPromise]);
  });
...
promise_test(_async(function (t) {
  ...
  var read = reader.read(new Uint8Array(16));
  return _await(pullCalledPromise, function () {
    resolvePull();
    return _await(delay(0), function () {
      reader.releaseLock();
      return _awaitIgnored(promise_rejects_js(t, TypeError, read, 'pending read should reject'));
    });
  });
...
promise_test(() => {
  ...
  var readPromise = rs.getReader().read();
  var br = controller.byobRequest;
  controller.close();
  br.respond(0);
  return readPromise;
...
test(() => {
  assert_throws_js(RangeError, () => new ReadableStream({
    type: 'bytes'
  }, {
    size() {
      return 1;
  ...
  var HasSizeMethod = /*#__PURE__*/function () {
    function HasSizeMethod() {
      _classCallCheck(this, HasSizeMethod);
    }
    return _createClass(HasSizeMethod, [{
      key: "size",
      value: function size() {}
    }]);
  }();
  assert_throws_js(RangeError, () => new ReadableStream({
    type: 'bytes'
  }, new HasSizeMethod()), 'constructor should throw when size on the prototype chain');
}, 'ReadableStream constructor should not accept a strategy with a size defined if type is "bytes"');
promise_test(_async(function (t) {
  var stream = new ReadableStream({
    pull: t.step_func(c => {
      var view = new Uint8Array(c.byobRequest.view.buffer, 0, 1);
      view[0] = 1;
  ...
  return _await(reader.read(new Uint8Array([4, 5, 6])), function (result) {
    assert_false(result.done, 'result.done');
    var view = result.value;
    assert_equals(view.byteOffset, 0, 'result.value.byteOffset');
...
promise_test(_async(function (t) {
  var stream = new ReadableStream({
    pull: t.step_func(c => {
      var view = new Uint8Array(c.byobRequest.view.buffer, 0, 0);
      c.close();
      c.byobRequest.respondWithNewView(view);
    }),
    type: 'bytes'
...
promise_test(_async(function (t) {
  ...
  return _await(pullCalledPromise, function () {
    ...
    var transferredView = transferArrayBufferView(controller.byobRequest.view);
    var newView = transferredView.subarray(0, 0);
    controller.close();
    controller.byobRequest.respondWithNewView(newView);
    return _await(readPromise, function (result) {
      assert_true(result.done, 'result.done');
...
promise_test(_async(function (t) {
  ...
  return _call(flushAsyncEvents, function () {
    ...
    return _await(read1, function (result1) {
      ...
      var read2 = reader2.read(new Uint8Array([4, 5, 6]));
      assert_equals(pullCount, 2, 'pull() must have been invoked twice');
...
promise_test(_async(function (t) {
  ...
  return _call(flushAsyncEvents, function () {
    ...
    var read1 = reader1.read(new Uint8Array([1, 2, 3]));
    var byobRequest1 = controller.byobRequest;
    ...
    return _await(promise_rejects_js(t, TypeError, read1, 'pending read must reject after releaseLock()'), function () {
...
promise_test(_async(function (t) {
  ...
  return _call(flushAsyncEvents, function () {
    ...
    var read2 = reader2.read(new Uint16Array(1));
    assert_not_equals(controller.byobRequest, null, 'byobRequest should not be invalidated after releaseLock()');
    assert_equals(controller.byobRequest, byobRequest1, 'byobRequest should be unchanged');
    assert_array_equals([...new Uint8Array(byobRequest1.view.buffer)], [1, 2, 3], 'byobRequest.view.buffer should be unchanged');
    return _await(promise_rejects_js(t, TypeError, read1, 'pending read must reject after releaseLock()'), function () {
      ...
      return _await(read2, function (result2) {
        ...
        var dataView2 = new DataView(view2.buffer, view2.byteOffset, view2.byteLength);
...
promise_test(_async(function (t) {
  ...
  return _call(flushAsyncEvents, function () {
    ...
    var read2 = reader2.read(new Uint8Array([4, 5]));
    assert_not_equals(controller.byobRequest, null, 'byobRequest should not be invalidated after releaseLock()');
    assert_equals(controller.byobRequest, byobRequest1, 'byobRequest should be unchanged');
    assert_array_equals([...new Uint8Array(byobRequest1.view.buffer)], [1, 2, 3], 'byobRequest.view.buffer should be unchanged');
    return _await(promise_rejects_js(t, TypeError, read1, 'pending read must reject after releaseLock()'), function () {
      ...
      return _await(read2, function (result2) {
        ...
        return _await(reader2.read(new Uint8Array([0, 0, 0])), function (result3) {
...
promise_test(_async(function (t) {
  ...
  return _call(flushAsyncEvents, function () {
    ...
    var read1 = reader1.read(new Uint16Array(1));
    var byobRequest1 = controller.byobRequest;
    assert_not_equals(byobRequest1, null, 'first byobRequest should exist');
  ...
  return _call(flushAsyncEvents, function () {
    ...
    return _await(promise_rejects_js(t, TypeError, read1, 'pending read must reject after releaseLock()'), function () {
      ...
      return _await(read2, function (result2) {
        ...
        return _await(reader2.read(), function (result3) {
          assert_false(result3.done, 'third result.done');
...
promise_test(_async(function (t) {
  ...
  var stream = new ReadableStream({
    ...
    autoAllocateChunkSize: 10,
    pull: t.step_func(c => {
      var newView = new Uint8Array(c.byobRequest.view.buffer, 0, 3);
      newView.set([20, 21, 22]);
      c.byobRequest.respondWithNewView(newView);
    })
  });
  var reader = stream.getReader();
  return _await(reader.read(), function (result) {
    assert_false(result.done, 'result.done');
    var view = result.value;
    assert_equals(view.byteOffset, 0, 'result.value.byteOffset');
