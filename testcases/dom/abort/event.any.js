...
test(t => {
  ...
  s.addEventListener("abort", t.step_func(e => {
...
test(t => {
  ...
  assert_equals(controller.signal, signal, "value of controller.signal should not have changed");
  controller.abort();
  assert_equals(controller.signal, signal, "value of controller.signal should still not have changed");
...
test(t => {
  ...
  controller.abort();
  assert_true(signal.aborted);
  assert_equals(eventCount, 1, "event handler should not have been called again");
}, "controller.abort() should do nothing the second time it is called");
test(t => {
  var controller = new AbortController();
  controller.abort();
  controller.signal.onabort = t.unreached_func("event handler should not be called");
...
test(t => {
  var reason = Error("hello");
  var signal = AbortSignal.abort(reason);
  assert_true(signal.aborted, "signal.aborted");
  assert_equals(signal.reason, reason, "signal.reason");
...
test(t => {
  ...
  assert_equals(reason.message, message, "abort.reason should not be changed by throwIfAborted()");
}, "throwIfAborted() should throw abort.reason if signal aborted");
test(t => {
  var signal = AbortSignal.abort('hello');
  assert_true(signal.aborted);
...
test(t => {
  ...
  assert_equals(signal.reason, signal.reason, "signal.reason returns the same DOMException");
}, "AbortSignal.reason returns the same DOMException");
test(t => {
  var controller = new AbortController();
  controller.abort();
  assert_true(controller.signal.reason instanceof DOMException, "signal.reason is a DOMException");
  assert_equals(controller.signal.reason, controller.signal.reason, "signal.reason returns the same DOMException");
